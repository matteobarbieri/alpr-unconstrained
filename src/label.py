...
class Label:
    ...
    def __init__(self, cl=-1,
              tl=np.array([0.,0.]), br=np.array([0.,0.]),
              prob=None, category=None):
        """
        cl_str : str
...
def lwrite(file_path, labels, write_probs=True, write_category_names=False):
    with open(file_path,'w') as fd:
        for l in labels:
            cc, wh, cl, prob, category_name = \
                (l.cc(),l.wh(),l.cl(),l.prob(), l.category())
            if prob != None and write_probs:
                fd.write('%d %f %f %f %f %f\n' % (cl,cc[0],cc[1],wh[0],wh[1],prob))
            else:
                if write_category_names:
                    fd.write('%d %f %f %f %f %s\n' % (
                        cl, cc[0], cc[1], wh[0], wh[1], category_name))
                else:
